...
export default class Logic {
	...
	private upgrade(esItem: EnchantmentItem) {
		...
		if (applyPity) {
			...
			if (roll < uppChance) {
				...
				if (esItem.name == EnchantmentItem.Reblath_Duo.name) this.duoOver30Active = false;
	...
	*/
	...
	public duoOver30_OnChange(oldDuoOver30: boolean, newDuoOver30: boolean): void {
		if (newDuoOver30) Logger.debug(`now use DuoOver30`);
		else Logger.debug(`now doesn't use DuoOver30`);
	}
	public limitDuos_OnChange(oldLimitDuos: number, newLimitDuos: number) {
	...
	public clicksPerHour_OnChange(newClicksPerHour: number) {
